...
Future<T?> imageLcPickerBottomSheet<T>(BuildContext context,
    ...
    required Function(AssetPathEntity album, AssetEntity image)? onSendTap}) {
  return showIzBottomSheet<T>(
      context: context,
      width: MediaQuery.sizeOf(context).width,
      height: MediaQuery.sizeOf(context).height * 0.7,
...
class _ImagePickerState extends State<ImagePicker> {
  ...
  Widget build(BuildContext context) {
    ...
    return LcScaffold(
      ...
      body: noImages
          ? const Center(child: Text(Localization.noImages))
          : Column(
...
class ImageLcPicker extends StatefulWidget {
  final Function(AssetPathEntity album, AssetEntity image)? onImageTap;
  final Function(AssetPathEntity album, AssetEntity image)? onSendTap;
  const ImageLcPicker({
    super.key,
    this.onImageTap,
    this.onSendTap,
              ...
              child: LcButton(
                ...
                onPressed: () {
                  if (widget.onSendTap != null) {
                    widget.onSendTap!(currentAlbum!, ChatController.instance().selectedImage.value!);
                  }
                },
              ),
            )
                                          ...
                                          child: InkWell(
                                            ...
                                            child: Container(
                                              decoration: BoxDecoration(
                                                  border: Border.all(
                                                      color: ChatController.instance().selectedImage.value == currentPhotos![i]
                                                          ? Colors.blue
                                                          : Colors.transparent)),
                                              child: AssetEntityImage(
                                                currentPhotos![i],
